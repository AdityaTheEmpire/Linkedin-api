...
class Linkedin(object):
    ...
    def get_job_skills(self, job_id: str) -> Dict:
        ...
        return data
    def get_company_details_and_posts(company_id, access_token):
     """
     Fetches LinkedIn company details and recent posts for a given company ID.
 
     Parameters:
     - company_id (str): The LinkedIn Company ID.
     - access_token (str): OAuth2 access token with necessary permissions.
    ...
    company_url = f'https://api.linkedin.com/v2/organizations/{company_id}'
    company_response = requests.get(company_url, headers=headers)

    if company_response.status_code != 200:
        raise Exception(f'Error fetching company details: {company_response.status_code}, {company_response.text}')
    ...
    # Endpoint to fetch company's posts
    posts_url = f'https://api.linkedin.com/v2/posts?q=authors&authors=urn:li:organization:{company_id}'
    posts_response = requests.get(posts_url, headers=headers)

    if posts_response.status_code != 200:
        raise Exception(f'Error fetching company posts: {posts_response.status_code}, {posts_response.text}')
